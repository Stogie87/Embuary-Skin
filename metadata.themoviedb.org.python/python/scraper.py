...
def search_for_movie(title, year, handle, settings):
    log(f"Find movie with title '{title}' from year '{year}'", xbmc.LOGINFO)
...
def _searchresult_to_listitem(movie):
    ...
    if movie.get('poster_path'):
        listitem.setArt({'poster': movie['poster_path']})
...
def set_info(infotag: xbmc.InfoTagVideo, info_dict):
    ...
    infotag.setStudios(info_dict['studio'] if isinstance(info_dict['studio'], list) else [info_dict['studio']])
    infotag.setGenres(info_dict['genre'] if isinstance(info_dict['genre'], list) else [info_dict['genre']])
    infotag.setCountries(info_dict['country'] if isinstance(info_dict['country'], list) else [info_dict['country']])
    infotag.setWriters(info_dict['credits'] if isinstance(info_dict['credits'], list) else [info_dict['credits']])
    infotag.setDirectors(info_dict['director'] if isinstance(info_dict['director'], list) else [info_dict['director']])
...
def find_defaultrating(rating_dict):
    return next((key for key, value in rating_dict.items() if value.get('default')), None)
