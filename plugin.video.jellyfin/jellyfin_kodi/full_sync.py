...
class FullSync(object):
            ...
            for library_id in libraries:
                ...
                if library:
                    if library.media_type == "mixed":
                        ...
                        libraries_list = self.get_libraries()
                        boxsets = [
                            row.view_id
        ...
        if self.sync["Libraries"]:
            ...
            for library in self.get_libraries():
                if library.media_type in (
                    "movies", "tvshows", "musicvideos", "music", "mixed"
                ):
                    libraries.append({
                        "Id": library.view_id,
        ...
        for items in server.get_items(
            ...
            with self.video_database_locks() as (videodb, jellyfindb):
                ...
                for index, show in enumerate(items["Items"]):
                    percent = int(
                        (float(start_index + index) / float(items["TotalRecordCount"])) * 100
                    )
                    message = show["Name"]
                    dialog.update(
                        percent,
                        ...
                        for episodes in server.get_episode_by_show(show["Id"]):
                            for episode in episodes["Items"]:
                                if episode.get("Path"):
                                    dialog.update(
                                        percent,
                                        message="%s/%s" % (message, episode["Name"][:10]),
                                    )
                                    obj.episode(episode)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        self.running = False
        window("jellyfin_sync.bool", clear=True)
