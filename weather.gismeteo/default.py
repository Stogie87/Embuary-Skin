...
def set_item_info(props, item, item_type, icon='%s.png', day_temp=None):
    ...
    if 'Outlook' in keys:
        props['Outlook'] = item.get('description', '')
    if 'Condition' in keys:
        props['Condition'] = item.get('description', '')
    if 'OutlookIcon' in keys:
        props['OutlookIcon'] = icon % weather_code
    if 'FanartCode' in keys:
        props['FanartCode'] = weather_code
    if 'ProviderIcon' in keys and weather.use_provider_icon:
        props['ProviderIcon'] = 'resource://resource.images.weatherprovidericons.gismeteo/{0}.png'.format(item.get('icon', ''))

    # Wind
    wind = item.get('wind', {})
    speed = 0
    wspeed = wind.get('speed', 0)
    if isinstance(wspeed, dict):
        speed = wspeed.get('avg', 0)
    elif isinstance(wspeed, float) or isinstance(wspeed, int):
        speed = wspeed
    if 'Wind' in keys:
    ...
    if 'WindDirection' in keys:
        props['WindDirection'] = weather.get_wind_direction(wind.get('direction', ''))
    if 'MaxWind' in keys:
        props['MaxWind'] = '{0} {1}'.format(
            weather.SPEED(safeget(wind, 'speed', 'max', default=0)),
            _(weather.SPEEDUNIT))
    ...
    if 'DewPoint' in keys and item_type == 'day':
        props['DewPoint'] = weather.DEW_POINT(temp.get('max', 0), safeget(humidity, 'avg', default=0)) + weather.TEMPUNIT
    elif 'DewPoint' in keys and item_type == 'hour':
        props['DewPoint'] = weather.DEW_POINT(temp.get('air', 0), humidity) + weather.TEMPUNIT
    elif 'DewPoint' in keys and item_type == 'cur':
        props['DewPoint'] = weather.DEW_POINT(temp.get('air', 0), humidity, False)
    if 'HighTemp' in keys:
        props['HighTemp'] = '{0}'.format(temp.get('max', 'n/a'))
    if 'LowTemp' in keys:
        props['LowTemp'] = '{0}'.format(temp.get('min', 'n/a'))
    if 'HighTemperature' in keys:
        props['HighTemperature'] = weather.TEMP(temp.get('max', 0)) + weather.TEMPUNIT
    if 'LowTemperature' in keys:
        props['LowTemperature'] = weather.TEMP(temp.get('min', 0)) + weather.TEMPUNIT
    if 'Temperature' in keys and item_type == 'cur':
        props['Temperature'] = temp.get('air', 'n/a')
    elif 'Temperature' in keys:
        props['Temperature'] = weather.TEMP(temp.get('air', 0)) + weather.TEMPUNIT
    if 'FeelsLike' in keys and item_type == 'cur':
        props['FeelsLike'] = temp.get('comfort', '')
    elif 'FeelsLike' in keys:
        props['FeelsLike'] = weather.TEMP(temp.get('comfort', 0)) + weather.TEMPUNIT

    if day_temp is not None:
        if 'TempMorn' in keys and day_temp.get('morn') is not None:
            props['TempMorn'] = weather.TEMP(day_temp['morn']) + weather.TEMPUNIT
        if 'TempDay' in keys and day_temp.get('day') is not None:
            props['TempDay'] = weather.TEMP(day_temp['day']) + weather.TEMPUNIT
        if 'TempEve' in keys and day_temp.get('eve') is not None:
            props['TempEve'] = weather.TEMP(day_temp['eve']) + weather.TEMPUNIT
        if 'TempNight' in keys and day_temp.get('night') is not None:
            props['TempNight'] = weather.TEMP(day_temp['night']) + weather.TEMPUNIT

    # Humidity
    if 'Humidity' in keys:
        hum = safeget(humidity, 'avg', default=humidity) if item_type == 'day' else humidity
        tpl = '{0}%' if item_type != 'cur' else '{0}'
        props['Humidity'] = tpl.format(hum) if hum is not None else _('n/a')
    if 'MinHumidity' in keys and item_type == 'day':
        hum = safeget(humidity, 'min', default=None)
        props['MinHumidity'] = '{0}%'.format(hum) if hum is not None else _('n/a')
    if 'MaxHumidity' in keys and item_type == 'day':
        hum = safeget(humidity, 'max', default=None)
        props['MaxHumidity'] = '{0}%'.format(hum) if hum is not None else _('n/a')

    # Pressure
    pressure = item.get('pressure', {})
    if 'Pressure' in keys:
        press = safeget(pressure, 'avg', default=pressure) if item_type == 'day' else pressure
        props['Pressure'] = '{0} {1}'.format(
            weather.PRESSURE(press),
            _(weather.PRESUNIT)) if press is not None else _('n/a')

    # Precipitation
    precip = item.get('precipitation', {})
    if 'Precipitation' in keys:
        props['Precipitation'] = '{0} {1}'.format(
            weather.PRECIPITATION(precip.get('amount', 0)),
            _(weather.PRECIPUNIT)) if precip.get('amount') is not None else _('n/a')
...
def set_location_props(forecast_info):
    ...
    set_item_info(current_props, forecast_info.get('current', {}), 'cur')
    location_info = Location(forecast_info)
    current_props['Location'] = getattr(location_info, 'name', 'Unbekannt')
    weather.set_properties(current_props, 'Current')

    # Forecast
    forecast_props = weather.prop_forecast()
    forecast_props['City'] = forecast_info.get('name', '')
    forecast_props['Country'] = forecast_info.get('country', '')
    forecast_props['State'] = forecast_info.get('district', '')
    forecast_props['Latitude'] = forecast_info.get('lat', '')
    forecast_props['Longitude'] = forecast_info.get('lng', '')
    forecast_props['Updated'] = weather.convert_date(forecast_info.get('cur_time', {}))
    weather.set_properties(forecast_props, 'Forecast')

    # Today
    today_props = weather.prop_today()
    sunrise = safeget(forecast_info, 'current', 'sunrise', 'unix', default=None)
    sunset = safeget(forecast_info, 'current', 'sunset', 'unix', default=None)
    if sunrise != sunset and sunrise is not None and sunset is not None:
        today_props['Sunrise'] = weather.get_time(safeget(forecast_info, 'current', 'sunrise', default={}))
        today_props['Sunset'] = weather.get_time(safeget(forecast_info, 'current', 'sunset', default={}))
    weather.set_properties(today_props, 'Today')

    for day in forecast_info.get('days', []):
        day_temp = None
        if day.get('hourly') is not None:
            day_temp = {}
            for hour in day.get('hourly', []):
                if hour.get('tod') == 0:
                    day_temp['night'] = safeget(hour, 'temperature', 'air', default=None)
                elif hour.get('tod') == 1:
                    day_temp['morn'] = safeget(hour, 'temperature', 'air', default=None)
                elif hour.get('tod') == 2:
                    day_temp['day'] = safeget(hour, 'temperature', 'air', default=None)
                elif hour.get('tod') == 3:
                    day_temp['eve'] = safeget(hour, 'temperature', 'air', default=None)
                # Hourly
                if count_hourly < MAX_HOURLY and safeget(hour, 'date', 'unix', default=0) >= CURRENT_TIME['unix']:
                    hourly_props = weather.prop_hourly()
                    set_item_info(hourly_props, hour, 'hour', WEATHER_ICON)
                    weather.set_properties(hourly_props, 'Hourly', count_hourly + 1)
                    count_hourly += 1
                # 36Hour
                if count_36hour < MAX_36HOUR and hour.get('tod') in [2, 3]:
                    if (hour.get('tod') == 2 and safeget(hour, 'date', 'unix', default=0) >= CURRENT_TIME['unix']) or hour.get('tod') == 3:
                        _36hour_props = weather.prop_36hour()
    ...
    if keyword:
        try:
            ...
            for s_location in search_result:
                ...
                item = {'id': getattr(location, 'id', ''), 'name': getattr(location, 'name', '')}
                locations.append(item)
                labels.append(getattr(location, 'label', item['name']))
            if locations:
...
def get_location(loc_id):
    ...
    if loc_id == '1' and use_current_location:
        try:
            lang = weather.gismeteo_lang()
            ip_locations = _ip_locations(lang)
        except (GismeteoError, WebClientError) as e:
        ...
        if not location_id and int_loc_id != 1:
            int_loc_id = 1
            location_id = weather.get_setting('Location{0}ID'.format(int_loc_id))
        location_name = weather.get_setting('Location{0}'.format(int_loc_id))
        data = {'name': location_name, 'id': location_id}
