...
class SimpleCache(object):
    ...
    def _get_mem_cache(self, endpoint, checksum, cur_time, json_data):
        ...
        if cachedata:
            try:
                ...
                self._log_msg("Error reading mem cache: %s" % exc, xbmc.LOGWARNING)
        return result
    ...
    def _get_db_cache(self, endpoint, checksum, cur_time, json_data):
        ...
        if cache_data is not None:
            try:
                ...
                self._log_msg("Error reading db cache: %s" % exc, xbmc.LOGWARNING)
        return result
    ...
    def _do_cleanup(self):
        ...
        db_result = self._execute_sql(query)
        if db_result is not None:
            try:
                for cache_data in db_result.fetchall():
                    ...
                    if cache_expires < cur_timestamp:
                        del_query = 'DELETE FROM simplecache WHERE id = ?'
                        self._execute_sql(del_query, (cache_id,))
                        self._log_msg("delete from db %s" % cache_id)
            except Exception as exc:
                self._log_msg("Error during cleanup: %s" % exc, xbmc.LOGWARNING)

        # compact db
        self._execute_sql("VACUUM")

        # remove task from list
        if __name__ in self._busy_tasks:
    ...
    def _execute_sql(self, query, data=None):
        ...
        if db is None:
            self._log_msg("_database ERROR ! -- DB connection is None", xbmc.LOGWARNING)
            return None
        with db:
            while not retries == 10 and not self._monitor.abortRequested():
                ...
                try:
                    if isinstance(data, list):
                        result = db.executemany(query, data)
                    elif data:
                    ...
                    if "_database is locked" in str(error):
