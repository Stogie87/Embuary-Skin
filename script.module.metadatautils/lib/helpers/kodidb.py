...
class KodiDb(object):
    ...
    def create_listitem(item, as_tuple=True, offscreen=True):
        ...
        try:
            ...
            # extra properties
            for key, value in item.get("extraproperties", {}).items():
                liz.setProperty(key, value)

            # InfoTagVideo-API für Video-Metadaten
            if nodetype == "Video":
                info = liz.getVideoInfoTag()
                info.setTitle(item.get("title", ""))
                info.setOriginalTitle(item.get("originaltitle", ""))
                info.setSortTitle(item.get("sorttitle", ""))
                info.setMediaType(item.get("type", ""))
                info.setIMDBNumber(item.get("imdbnumber", ""))
                info.setYear(item.get("year", 0))
                info.setPremiered(item.get("premiered", ""))
                info.setPlot(item.get("plot", ""))
                info.setPlotOutline(item.get("plotoutline", ""))
                info.setMpaa(item.get("mpaa", ""))
                info.setPlaycount(item.get("playcount", 0))
                info.setDbId(int(item.get("extraproperties", {}).get("DBID", 0)))
                try:
                    info.setRating(float(item.get("rating", 0)))
                except Exception:
                    pass
                try:
                    info.setDuration(int(item.get("duration", 0)))
                except Exception:
                    pass
                # Cast
                cast_list = []
                for cast in item.get("castandrole", []):
                    cast_list.append(xbmc.Actor(cast[0], cast[1]))
                info.setCast(cast_list)
                # Properties für alles, was nicht per Setter geht
                liz.setProperty('genre', item.get('genre', ''))
                liz.setProperty('studio', item.get('studio', ''))
                liz.setProperty('tagline', item.get('tagline', ''))
                liz.setProperty('writer', item.get('writer', ''))
                liz.setProperty('director', item.get('director', ''))
                liz.setProperty('status', item.get('status', ''))
                liz.setProperty('votes', str(item.get('votes', '')))
                liz.setProperty('trailer', item.get('trailer', ''))
                liz.setProperty('tvshowtitle', item.get('tvshowtitle', ''))
                liz.setProperty('album', item.get('album', ''))
                liz.setProperty('artist', " / ".join(item.get('artist', [])))
                liz.setProperty('aired', item.get('aired', ''))
                liz.setProperty('credits', item.get('credits', ''))
                liz.setProperty('season', str(item.get('season', '')))
                liz.setProperty('episode', str(item.get('episode', '')))
                liz.setProperty('country', item.get('country', ''))
                liz.setProperty('trailer', item.get('trailer', ''))
                liz.setProperty('votes', str(item.get('votes', '')))
                liz.setProperty('code', item.get('imdbnumber', ''))
                # und so weiter, je nach Bedarf

                # Streamdetails – NEU: InfoTagVideo.addVideoStream/addAudioStream/addSubtitleStream
                if item.get("streamdetails"):
                    ...
                    if subtitle:
                        info.addSubtitleStream(
                            language=subtitle.get("language", ""),
                            name=subtitle.get("name", "")
                        )

            # Music-Items: InfoTagMusic, Fallback auf setInfo
            else:
                try:
                    info = liz.getMusicInfoTag()
                    info.setTitle(item.get("title", ""))
                    info.setAlbum(item.get("album", ""))
                    info.setArtist(" / ".join(item.get('artist', [])))
                    info.setGenre(item.get("genre", ""))
                    info.setYear(item.get("year", 0))
                    info.setTrack(item.get("track", 0))
                    info.setDuration(item.get("duration", 0))
                    info.setPlaycount(item.get("playcount", 0))
                    info.setLyrics(item.get("lyrics", ""))
                    # Weitere Felder nach Bedarf ergänzen
                except Exception:
                    # Fallback für alte Kodi-Versionen
                    infolabels = {
