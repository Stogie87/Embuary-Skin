...
def picker_context(kind: str):
    ...
    previous = home.getProperty("skinshortcuts-picker")
    home.setProperty("skinshortcuts-picker", kind)
    try:
        yield
    finally:
        # Nested picker dialogs temporarily change the picker kind. Restore the
        # outer session marker instead of clearing it between hierarchy levels.
        if previous:
            home.setProperty("skinshortcuts-picker", previous)
        else:
...
class PickersMixin:
    ...
    def _choose_shortcut(self) -> None:
        ...
        # Keep the marker alive for the complete shortcut selection session,
        # including hierarchy recursion and add-on directory browsing.
        with picker_context("shortcut"):
